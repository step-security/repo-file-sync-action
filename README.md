...
In order for the Action to access your repositories you have to specify a [Personal Access token](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/creating-a-personal-access-token) as the value for `GH_PAT` (`GITHUB_TOKEN` will **not** work). The PAT needs the full repo scope ([#31](https://github.com/step-security/repo-file-sync-action/discussions/31#discussioncomment-674804)).
...
```yml
uses: step-security/repo-file-sync-action@latest
```
...
Here are all the inputs [repo-file-sync-action](https://github.com/step-security/repo-file-sync-action) takes:
...
In order to tell [repo-file-sync-action](https://github.com/step-security/repo-file-sync-action) what files to sync where, you have to create a `sync.yml` file in the `.github` directory of your main repository (see [action-inputs](#%EF%B8%8F-action-inputs) on how to change the location).
...
Created by Step Security (@StepSecurity)
```
...
By default [repo-file-sync-action](https://github.com/step-security/repo-file-sync-action) will add the `sync` label to every PR it creates. You can turn this off by setting `PR_LABELS` to false, or specify your own labels:
...
You can tell [repo-file-sync-action](https://github.com/step-security/repo-file-sync-action) to assign users to the PR with `ASSIGNEES`:
...
- name: Run GitHub File Sync
  ...
  with:
    GH_PAT: ${{ secrets.GH_PAT }}
    ASSIGNEES: StepSecurity
```

### Request a PR review

You can tell [repo-file-sync-action](https://github.com/step-security/repo-file-sync-action) to request a review of the PR from users with `REVIEWERS` and from teams with `TEAM_REVIEWERS`:
...
If your repo name contains invalid characters, like a dot ([#32](https://github.com/step-security/repo-file-sync-action/issues/32)), you can specify a different prefix for the branch (the text before `/SOURCE_BRANCH_NAME`):
...
Here's how I keep common files in sync across my repositories. The main repository [`github-files`](https://github.com/StepSecurity/github-files) contains all the files I want to sync and the [repo-file-sync-action](https://github.com/step-security/repo-file-sync-action) Action which runs on every push.
...
group:
  ...
  - files:
    ...
    repos: |
      step-security/do-spaces-action
      step-security/running-at
      step-security/spaces-cli
      step-security/metadata-scraper
      step-security/ejs-serve
      step-security/feedback-js
...
Copyright 2021 Step Securitymilian Schiller
